  ...
  <div>
    ...
    <main class="main">
      ...
      <div v-if="scene === 2">
        ...
        <p class="form-info center">{{ account }}</p>
      ...
      <div v-else>
        ...
        <div class="form-info center">{{ getAccount }}</div>
        <div class="divider"></div>
...
export default {
  ...
  computed: {
    ...
    getAccount() {
      ...
      this.account = this.accounts[this.selectedIndex].account;
      return this.account;
    },
  },
  methods: {
    ...
    submitForm() {
      ...
      decryptKeystore(keystore, this.password).then((result) => {
        if (!result) {
          this.$notify({
            group: "notify",
            type: "error",
