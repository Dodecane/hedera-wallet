  ...
  <header class="header">
    <div class="header-top">
      <router-link class="header-logo" to="/home">
        <img src="images/hedera-wallet-small.png" alt="Hedera" />
      ...
      <div class="network" v-click-outside="hideNetworkDropdown">
        <a
          v-show="this.headerTab === 'create-tab'"
          href="#"
        ...
        >
          ...
          <span class="network-name">{{
            trimNetworkName(currentNetwork.name)
          }}</span>
        ...
        <a
          ...
          class="network-toggle"
          v-tooltip.bottom="
            'Network can only be changed when creating/importing an account'
          "
        >
          <span class="network-icon">
            <svg
        ...
        <div class="network-dropdown" v-show="showNetworkDropdown">
          ...
          >
            <span class="network-dropdown-icon">
              <svg
                v-if="currentNetwork.name === network.name"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="-2 -1.5 24 24"
                width="24"
                height="24"
            ...
            <span>{{ trimNetworkName(network.name) }}</span>
          </a>
        </div>
      </div>
      ...
      <div v-click-outside="hideDropdownMenu">
        ...
        <nav class="dropdown-menu" v-show="showDropdownMenu">
            ...
            <div class="account-content" ref="accountcontent">
              ...
              >
                <i
                  ...
                  v-show="
                    account.publicKey === wallets.active.publicKey &&
                ...
                >
                  <a
                    @click.prevent="
                      () => {
                        selectAccount(account.publicKey);
                      }
                    "
          ...
          <div class="dropdown-menu-item">
            ...
            <router-link to="/about">About Hedera Wallet</router-link>
          </div>
          <div v-if="wallets.accounts.length > 0">
            <div class="dropdown-menu-divider"></div>
            <div class="dropdown-menu-item">
...
export default {
  ...
  async created() {
    const { availableNetworks } = await import("../../domain/network");
    this.networks = availableNetworks;
    if (this.wallets.accounts.length > 0) {
      this.changeNetwork(this.networks[this.wallets.active.network]);
    }
  },
  ...
  methods: {
    ...
    selectAccount(publicKey) {
      this.$store.commit("wallets/setActive", publicKey);
...
.dropdown-menu {
  box-shadow: 0 7px 14px rgba(22, 22, 22, 0.6);
  background: #f9f9f9;
  position: absolute;
  top: 40px;
  right: 10px;
